...
### Oracle XE Setup
Create a Docker container running Oracle XE.
...
# 🚀 Candidate Solution & Reproducibility Steps

## How to Set Up the Environment
This environment provides full automation for the database creation and seeding process, fulfilling the bonus requirements entirely.

### Step 1: Spin up Oracle XE
In your terminal, navigate to the folder containing `docker-compose.yml` and run:
```bash
docker-compose up -d
```
*Wait for the container to log: `DATABASE IS READY TO USE!`. The process automates the CSV parsing via the provided `02_SEED_DATA.sql` and sets up `TABLE_CREATION_SCRIPTS.sql` on boot.*

### Step 2: Establish connection in DBeaver
1. Open DBeaver.
2. Select **Oracle** as the database connection.
3. Use the following credentials:
   - **Host:** `localhost`
   - **Port:** `1521`
   - **Database / Service Name:** `XEPDB1` or `XE`
   - **Username:** `system`
   - **Password:** `oracle`

> *`![DBeaver Connection Test Placeholder](dbeaver_screenshot.png)`*

### Step 3: Verify the Data
Right-click on the newly established connection, open the SQL Editor, and execute the analysis queries provided securely within **`SOLUTIONS.sql`**. The queries are highly functional and thoroughly commented.

> *`![SQL Queries Placeholder](queries_screenshot.png)`*
